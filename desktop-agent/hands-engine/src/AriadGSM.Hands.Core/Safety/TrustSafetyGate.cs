...
public sealed class TrustSafetyGate
{
    ...
    public TrustSafetyGateDecision Evaluate()
    {
        ...
        {
            return new TrustSafetyGateDecision(true, "ALLOW", "Trust & Safety gate no bloquea modo plan.", 0);
        }

        if (!File.Exists(_options.TrustSafetyStateFile))
        {
            return new TrustSafetyGateDecision(false, "ASK_HUMAN", "Trust & Safety aun no publico permiso para manos.", int.MaxValue);
        }
        ...
        try
        {
            ...
            {
                return new TrustSafetyGateDecision(false, "ASK_HUMAN", $"Trust & Safety esta viejo ({ageMs} ms); no muevo manos.", ageMs);
            }

            if (!root.TryGetProperty("permissionGate", out var permissionGate))
            {
                return new TrustSafetyGateDecision(false, "ASK_HUMAN", "Trust & Safety no trae permissionGate.", ageMs);
            }
            ...
            if (!canHandsRun)
            {
                return new TrustSafetyGateDecision(false, decision, string.IsNullOrWhiteSpace(reason) ? "Trust & Safety pauso manos." : reason, ageMs);
            }

            return new TrustSafetyGateDecision(true, decision, string.IsNullOrWhiteSpace(reason) ? "Trust & Safety autorizo manos." : reason, ageMs);
        }
        catch (Exception exception)
        {
            return new TrustSafetyGateDecision(false, "ASK_HUMAN", $"No pude leer Trust & Safety: {exception.Message}", int.MaxValue);
        }
    }
